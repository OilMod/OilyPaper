OilyPaper a PaperMCFork for OilMod
======
An implementation of the [Bukkit](https://hub.spigotmc.org/stash/projects/SPIGOT/repos/bukkit) plugin API for [Minecraft](https://minecraft.net/) servers, currently maintained by [PaperMC](http://www.papermc.io/).
...
Bug Reporting
-------------
The development team is very open to both bug and feature requests / suggestions. You can submit these on the in the github issue tracker.
...
* Agree to our [license](./LICENSE.md)
* See [Readme](./README.md#pushing)
* Create push request including only the patch files

If you submit a PR involving both OilMod and OilyPaper, each PR should link the other.

Code has to be in Java 8.
...
* No 160 character column limit, or 'weird' mid-statement newlines unless absolutely necessary.
    * The 160 character column limit still applies to documentation.
* No one-line methods.
...
1. run `./paper` mcdev which is located inside of `./Paper/`
2. Find the class you want to add in the `/Paper/work/Minecraft/{version}/src/net/minecraft/server` folder and copy it.
3. Copy the selected file to the `src/main/java/net/minecraft/server` folder in OilyPaper.
...
```java
//OilSpigot start
//(THE REASON:) special case dropping so we can get info from the tile entity
public void dropNaturally(World world, int i, int j, int k, int l, float f, int i1) {
    if (world.random.nextFloat() < f) {
        ItemStack itemstack = new ItemStack(Item.SKULL, 1, this.getDropData(world, i, j, k));
        TileEntitySkull tileentityskull = (TileEntitySkull) world.getTileEntity(i, j, k);
